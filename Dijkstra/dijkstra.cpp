...
dijkstra::dijkstra(int startnode[],
        int goalnode[],
        cv::Mat map)
{
    mapCv = map;
...
void dijkstra::build_motion_model(){
...
void dijkstra::explore_one_node(std::array<int, 2> curr_xy){
    // move it into closed 
...
std::vector< std::array<int, 2>> dijkstra::find_min_cost_nodes(){
...
void dijkstra::explore_one_ite(std::vector< std::array<int, 2>> active_nodes){
...
std::deque< std::array<int, 2>> dijkstra::get_path(){
...
void dijkstra::get_path_no_return(){
...
void dijkstra::draw_start_goal_on_map(){
