...
private:
    ...
    const float k_max_step_length = 50;
    const float k_near_node_range = 30;

    const int k_max_num_sample = 1000;
    ...
    void sample_a_point( int ite , bool use_goal);
...
public:
    // RRT_star(int startnode[], int goalnode[],  cv::Mat map);
    RRT_star( std::array<int, 2> &startnode, std::array<int, 2> &goalnode,  cv::Mat map);
